...
public:
	 ...
	 */
	[[nodiscard]] const Address &getAddress() const noexcept { return address; }

	/**
	 * @brief Obtains gRPC channel credentials
	 * @return
	 */
	[[nodiscard]] const Creadentials &getCredentials() const noexcept { return credentials; }
	/**
	 ...
	 */
	Options &setCredentials(Creadentials &&creds) noexcept(false)
	{
		if(!credentials) {
	 ...
	 */
	[[nodiscard]] Duration getSingularCallDeadline() const noexcept { return singular_call_deadline; }
	/**
	 ...
	 */
	Options &setSingularCallDeadline(Duration deadline) noexcept(false)
	{
	 ...
	 * @return Default deadline value
	 */
	[[nodiscard]] Duration getServerStreamDeadline() const noexcept { return server_stream_deadline; }
	/**
	 ...
	 * @throws std::invalid_argument if deadline is less than 10 msec
	 */
	Options &setServerStreamDeadline(Duration deadline) noexcept(false)
	{
		if(deadline.count() < 10) {
			throw std::invalid_argument("invalid call deadline, should be greater than 10 msec");
		}
	 ...
	 */
	[[nodiscard]] Duration getServerStreamRelaunchInterval() const noexcept { return server_stream_relaunch_interval; }
	/**
	 ...
	 */
	Options &setServerStreamRelaunchInterval(Duration interval) noexcept(false)
	{
	 ...
	 */
	[[nodiscard]] ServerStreamRelaunchPolicy getServerStreamRelaunchPolicy() const noexcept
	{
	 ...
	 * @return Chaining ref
	 */
	Options &setServerStreamRelaunchPolicy(ServerStreamRelaunchPolicy policy) noexcept
	{
	 ...
	 */
	[[nodiscard]] SizeLimitBytes getRequestSizeLimitBytes() const noexcept { return request_size_limit_bytes; }
	 ...
	 * @return Chaining ref
	 */
	Options &setRequestSizeLimitBytes(SizeLimitBytes limit)
	{
		if(limit && *limit <= 0) {
	 ...
	 * @return Empty, if no limit is set
	 */
	[[nodiscard]] SizeLimitBytes getResponseSizeLimitBytes() const noexcept { return response_size_limit_bytes; }
	 ...
	 * @return Chaining ref
	 */
	Options &setResponseSizeLimitBytes(SizeLimitBytes limit)
	{
		if(limit && *limit <= 0) {
			throw std::invalid_argument("limit should be positive: " + std::to_string(*limit));
		}
